...
<hr>
   ...
   // To make Pace works on Ajax calls
    ...
    $('.ajax').click(function(){
        $.ajax({url: '<?php echo $this->_config['ACTION_URL'];?>/index/get_results', success: function(result){
            $('.ajax-content').html('<h4>返回结果</h4><hr>'+getTime()+'<hr>'+result);
        }});
     });
    $('.get_agent_list').click(function(){
        $.ajax({url: '<?php echo $this->_config['ACTION_URL'];?>/index/get_agent_list', success: function(result){
            $('.ajax-content').html('<h4>返回结果</h4><hr>'+getTime()+'<hr>'+result);
        }});
     });
    ...
    $('.register_agent').click(function(){
        $.ajax({url: '<?php echo $this->_config['ACTION_URL'];?>/index/register_agent?agent_name='+$('#register_agent_agent_name').val()+'&agent_ip='+$('#register_agent_agent_ip').val(), success: function(result){
            $('.ajax-content').html('<h4>返回结果</h4><hr>'+getTime()+'<hr>'+result);
        }});
     });
    $('.unregister_agent').click(function(){
        $.ajax({url: '<?php echo $this->_config['ACTION_URL'];?>/index/unregister_agent?agent_id='+$('#unregister_agent_agent_id').val(), success: function(result){
            $('.ajax-content').html('<h4>返回结果</h4><hr>'+getTime()+'<hr>'+result);
        }});
     });
     $('.update_agent_status').click(function(){
        $.ajax({url: '<?php echo $this->_config['ACTION_URL'];?>/index/update_agent_status', success: function(result){
            $('.ajax-content').html('<h4>返回结果</h4><hr>'+getTime()+'<hr>'+result);
        }});
     });
     
     $('.plugin_running_status').click(function(){
        $.ajax({url: '<?php echo $this->_config['ACTION_URL'];?>/index/plugin_running_status?agent_id='+$('#plugin_running_status_agent_id').val(), success: function(result){
            $('.ajax-content').html('<h4>返回结果</h4><hr>'+getTime()+'<hr>'+result);
        }});
     });
     
     $('.is_alive').click(function(){
        $.ajax({url: '<?php echo $this->_config['ACTION_URL'];?>/index/is_alive?agent_id='+$('#is_alive_agent_id').val(), success: function(result){
            $('.ajax-content').html('<h4>返回结果</h4><hr>'+getTime()+'<hr>'+result);
        }});
     });
